  ...
  return (
    <div className="space-y-5">
      ...
      <section className="rounded-2xl border border-line bg-card p-4 shadow-soft">
          ...
          {rows.map((r) => (
            <li key={r.id} className="flex items-center justify-between gap-2 py-3">
              <div className="min-w-0">
                <p className="truncate text-sm text-ink">{r.memo || r.type}</p>
                <p className="text-[11px] text-muted">{r.date} · {r.created_by}</p>
              </div>
              <div className="flex shrink-0 items-center gap-3">
                <span className={`text-sm font-medium ${r.type === '입금' ? 'text-plus' : 'text-minus'}`}>{r.type === '입금' ? '+' : '−'}{won(r.amount)}</span>
                {isEditor && (
        ...
        {!isEditor && <p className="mt-2 border-t border-line pt-3 text-xs text-muted">내역 기록은 {EDITOR} 님이 합니다.</p>}
      </section>
    </div>
  )
}
